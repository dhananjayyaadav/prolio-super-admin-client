...
const InfluencerPage = () => {
  ...
  const fetchInfluencerCounts = async () => {
    try {
      const response = await api.get("/admin/influencers/all-influencers");
      if (response.data.success) {
        const counts = {
          verified: response.data.influencers.filter(
            (inf) => inf.isInfluencer?.verified
          ).length,
          rejected: response.data.influencers.filter(
            (inf) => inf.isInfluencer?.rejected
          ).length,
          pending: response.data.influencers.filter(
            (inf) => !inf.isInfluencer?.verified && !inf.isInfluencer?.rejected
          ).length,
        };
      ...
      if (response.data.success) {
        ...
        switch (activeTab) {
          ...
          case "pending":
            filteredInfluencers = filteredInfluencers.filter(
              (inf) =>
                !inf.isInfluencer?.verified && !inf.isInfluencer?.rejected
            );
            break;
          default:
            break;
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        ...
        const response = await api.patch(
          `/admin/influencers/updateInfluencer-status/${influencerId}`,
          requestBody
        );
                        ...
                        <StatusBadge
                          status={
                            influencer.isInfluencer?.verified
                              ? "verified"
                              : influencer.isInfluencer?.rejected
                              ? "rejected"
                              : "pending"
                          }
                        />
