  ...
  const updateSubCategory = (subCategoryId, value) => {
    setFormStructure((prev) => ({
      ...
      subCategories: prev.subCategories.map((sc) =>
        sc.id === subCategoryId ? { ...sc, name: value } : sc
      ),
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5007/admin/category/add-category",
        formStructure
      );
      ...
      toast.error(error.response?.data?.error || "Error saving form");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full mx-auto p-6">
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Create a New Category Form Structure
          </p>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          ...
          {formStructure.subCategories.length === 0 && (
            <div className="text-center py-6 bg-gray-50 rounded-lg border-2 border-dashed">
              <p className="text-gray-500">No sub-categories added yet</p>
              <button
                onClick={addSubCategory}
                className="mt-2 text-sm text-blue-600 hover:text-blue-700"
              >
